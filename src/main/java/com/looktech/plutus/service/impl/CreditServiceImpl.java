...
public class CreditServiceImpl implements CreditService {
    ...
    @Transactional
    @CacheEvict(value = "userBalance", key = "#userId")
    public List<CreditLedger> reserveCredit(Long userId, BigDecimal amount) {
        if (amount.compareTo(BigDecimal.ZERO) <= 0) {
            throw new CreditException("INVALID_AMOUNT", "Credit amount must be positive");
        }
    ...
    public CreditTransactionLog cancelReservation(String transactionId) {
        CreditTransactionLog transactionLog = transactionLogRepository.findByTransactionId(transactionId)
        ...
        cancelLog.setUserId(transactionLog.getUserId());
        cancelLog.setTransactionId(transactionId);
        cancelLog.setType(CreditTransactionLog.TransactionType.CANCEL);
        cancelLog.setAmount(transactionLog.getAmount());
        cancelLog.setSourceType(transactionLog.getSourceType());
        cancelLog.setSourceId(transactionLog.getSourceId());
        return transactionLogRepository.save(cancelLog);
    }

    @Override
    @Transactional
    @CacheEvict(value = "userBalance", key = "#result.userId")
    public CreditTransactionLog settleCredit(String transactionId) {
        CreditTransactionLog transactionLog = transactionLogRepository.findByTransactionId(transactionId)
                .orElseThrow(() -> new CreditException("TRANSACTION_NOT_FOUND", "Transaction not found"));
        ...
        consumeLog.setUserId(transactionLog.getUserId());
        consumeLog.setTransactionId(transactionId);
        consumeLog.setType(CreditTransactionLog.TransactionType.CONSUME);
        consumeLog.setAmount(transactionLog.getAmount());
        consumeLog.setSourceType(transactionLog.getSourceType());
        consumeLog.setSourceId(transactionLog.getSourceId());
        consumeLog.setCreditId(reservedLedgers.get(0).getId());
